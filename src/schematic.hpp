...
class Schematic {
	...
	struct ConnectionRaw {
		...
		ConnectionRaw(int _src, int _src_pin, int _dst, int _dst_pin) :src(_src), src_pin(_src_pin), dst(_dst), dst_pin(_dst_pin) {}
		bool IsValid() {
...
private:
	...
	Error ParseJson(const boost::json::value& js);
	Error ParseJsonBlock(const boost::json::value& js, Block* block);
	Error ParseJsonConnection(const boost::json::value& js);
